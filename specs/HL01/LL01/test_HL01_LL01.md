...
| Test | Scenario | AC |
|------|----------|-----|
| `test_error_paths_do_not_corrupt_state` | Shorten valid URL → attempt invalid shorten (verify error) → resolve original (still works) → attempt alias collision (verify error) → resolve original (still works) | AC-05, AC-06, AC-07, AC-09 |
