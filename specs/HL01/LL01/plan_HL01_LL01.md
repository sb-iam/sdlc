...
3. If code already in `_store`, regenerate in a loop until unique (collision probability ~1/56 billion per attempt). Only raises `ValueError` if the entire code space (~56.8 billion codes) is exhausted.
